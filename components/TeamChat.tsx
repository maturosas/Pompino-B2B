  ...
  return (
    <div className="flex h-[calc(100vh-140px)] md:h-[calc(100vh-180px)] bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl relative overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="w-64 border-r border-white/5 bg-black/40 flex flex-col">
          <div className="p-4 border-b border-white/5">
              ...
              <button 
                ...
                className="w-full py-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-lg text-[10px] font-bold uppercase text-white transition-all flex items-center justify-center gap-2"
              >
                  <span>+</span> Crea tu Chat
              </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scroll p-2 space-y-1">
              ...
              {channels.map(channel => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 rounded-lg text-xs font-medium transition-all flex items-center gap-2 ${activeChannelId === channel.id ? 'bg-indigo-600 text-white font-bold shadow-lg' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
                  >
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        {/* Channel Header */}
        <div className="p-4 border-b border-white/5 bg-white/[0.02] backdrop-blur-md flex justify-between items-center z-10">
            <div>
        ...
        {/* Input Area */}
        <form onSubmit={handleSend} className="p-3 bg-black border-t border-white/10 z-10">
            <div className="relative flex gap-2">
                <input 
                    type="text" 
                    value={inputText}
                    onChange={(e) => setInputText(e.target.value)}
                    placeholder={`Mensaje en #${channels.find(c => c.id === activeChannelId)?.name || 'General'}...`}
